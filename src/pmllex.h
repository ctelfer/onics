...
/*
 * Get the next token from input and its associated value returns 0 on EOF
 * and -1 on error.  val->name points to the current input name.  This string
 * is shared by all the tokens for the lexer.  Consequently this string should
 * be copied out if it is needed past the end of pmll_free() for the lexer
 * that produced it.  pmll_clear() should be called on val to release memory
 * for the token after it is no longer needed.
 */
