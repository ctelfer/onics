...
struct pdu *fld_get_pdui(struct pdu *plist, struct ns_namespace *ns,
			    uint idx);

/* as per fld_get_pdui() but look up the protocol by name */
struct pdu *fld_get_pduni(struct pdu *plist, const char *s, uint idx);

/* as per fld_get_pdui() but idx = 0 */
struct pdu *fld_get_pdu(struct pdu *plist, struct ns_namespace *ns);

/* as per fld_get_pduni() but idx = 0 */
...
void *fld_get_hdrn(byte_t *p, struct pdu *plist, const char *s,
		   ulong *len);

/* as fld_get_pldni() but with idx = 0 */
void *fld_get_pldn(byte_t *p, struct pdu *plist, const char *s,
		   ulong *len);

/* as fld_get_trlni() but with idx = 0 */
void *fld_get_trln(byte_t *p, struct pdu *plist, const char *s,
		   ulong *len);
 ...
 */
void *fld_get_pi(byte_t *p, struct pdu *plist, struct ns_pktfld *pf,
		 uint idx, ulong *len);

/* as fld_get_pi() but with idx == 0) */
void *fld_get_p(byte_t *p, struct pdu *plist, struct ns_pktfld *pf,
		ulong *len);
 ...
 */
int fld_get_vi(byte_t *p, struct pdu *plist, struct ns_pktfld *pf,
	       uint idx, ulong *v);
/*
 * read the value of the 'idx'th 'pf' field in a packet with
 ...
 * returns 0 on success and -1 on failure
 */
int fld_get_bi(byte_t *sp, struct pdu *plist, struct ns_pktfld *pf,
 ...
 * returns 0 on success and -1 on failure
 */
int fld_set_vi(byte_t *dp, struct pdu *plist, struct ns_pktfld *pf,
	       uint idx, ulong v);
 ...
 * returns 0 on success and -1 on failure
 */
int fld_set_bi(byte_t *dp, struct pdu *plist, struct ns_pktfld *pf,
	       uint idx, void *sp, size_t len);


/* as fld_get_vi(), but the field is looked up by name */
int fld_get_vni(byte_t *p, struct pdu *plist, const char *s, uint idx,
	        ulong *v);

/* as fld_get_bi(), but the field is looked up by name */
int fld_get_bni(byte_t *sp, struct pdu *plist, const char *s, uint idx,
	        void *dp, size_t len);

/* as fld_set_vi(), but the field is looked up by name */
int fld_set_vni(byte_t *p, struct pdu *plist, const char *s, uint idx,
	        ulong v);

/* as fld_set_bi(), but the field is looked up by name */
int fld_set_bni(byte_t *dp, struct pdu *plist, const char *s, uint idx,
	        void *sp, size_t len);


/* as fld_get_vi(), but index is implicitly 0 */
int fld_get_v(byte_t *p, struct pdu *plist, struct ns_pktfld *pf,
	      ulong *v);

/* as fld_get_bi(), but index is implicitly 0 */
int fld_get_b(byte_t *sp, struct pdu *plist, struct ns_pktfld *pf,
	      void *dp, size_t len);

/* as fld_set_vi(), but index is implicitly 0 */
int fld_set_v(byte_t *p, struct pdu *list, struct ns_pktfld *pf,
	      ulong v);

/* as fld_set_bi(), but index is implicitly 0 */
int fld_set_b(byte_t *dp, struct pdu *plist, struct ns_pktfld *pf,
	      void *sp, size_t len);


/* as fld_getnvi, but index is implicitly 0 */
int fld_get_vn(byte_t *p, struct pdu *plist, const char *s, ulong *v);

/* as fld_getnbi, but index is implicitly 0 */
int fld_get_bn(byte_t *sp, struct pdu *plist, const char *s,
	       void *dp, size_t len);

/* as fld_set_nvi, but index is implicitly 0 */
int fld_set_vn(byte_t *p, struct pdu *plist, const char *s, ulong v);

/* as fld_setn_bi, but index is implicitly 0 */
int fld_set_bn(byte_t *dp, struct pdu *plist, const char *s,
	       void *sp, size_t len);
...
struct npf_list {
	struct npfield 		list;	/* pdu == NULL, nse == NULL */
	struct pdu *		plist;
	byte_t *		buf;
	uint			nfields;
	uint			ngaps;
...
#define npf_is_pdu(npf)		((npf)->nse == NULL && (npf)->buf == NULL)
#define npf_type_eq(n1, n2)	(((n1)->nse == (n2)->nse) && \
				 (n1)->pdu->prid == (n2)->pdu->prid)
