...
int main(int argc, char *argv[])
{
	...
	E(ns_insert
	  (tcpns, (struct ns_element *)ns_new_bitfield("syn", 13 * 8 + 6, 1)));
	E(ns_insert
	  (tcpns, (struct ns_element *)ns_new_bitfield("ack", 13 * 8 + 3, 1)));
	E(ns_insert
	  (tcpns, (struct ns_element *)ns_new_bitfield("psh", 13 * 8 + 4, 1)));

	E(ns_insert(ns, (struct ns_element *)ns_new_scalar("http", 80)));
	E(ns_insert(ns, (struct ns_element *)ns_new_scalar("ssh", 22)));
	E(ns_insert
	  (ns, (struct ns_element *)ns_new_srange("reserved", 0, 1023)));
	...
	E(ns_insert
	  (ns, (struct ns_element *)ns_new_raw("addr0", arr2raw(addr0, rv))));
	E(ns_insert
	  (ns,
	   (struct ns_element *)ns_new_masked("maddr1", arr2raw(addr1, rv),
					      arr2raw(mask1, rv2))));
	E(ns_insert
	  (ns,
	   (struct ns_element *)ns_new_prefixed("paddr1", arr2raw(addr1, rv),
						24)));
	...
	E(!
	  (nsrv = (struct ns_rawval *)ns_name_lookup(ns, "addr0", NSTYPE_RAW)));
	...
	E(!
	  (m =
	   (struct ns_masked *)ns_name_lookup(ns, "maddr1", NSTYPE_MASKED)));
	...
	E(!
	  (m =
	   (struct ns_masked *)ns_name_lookup(ns, "paddr1", NSTYPE_MASKED)));
	if (ns_cmp_raw((struct ns_element *)m, saddr, 4))
	...
	E(!
	  (m =
	   (struct ns_masked *)ns_name_lookup(ns, "maddr2", NSTYPE_MASKED)));
	if (ns_cmp_raw((struct ns_element *)m, saddr, 4))
	...
	E(!
	  (r =
	   (struct ns_ranges *)ns_name_lookup(ns, "rraddr1", NSTYPE_RRANGE)));
